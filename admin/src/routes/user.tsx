...
const getUserTokens = async () => {
  ...
  tokensData.forEach((token) => {
    ...
    if (token.last_used_at !== null) {
      token.last_user_at = dayjs(token.last_used_at).format(
...
export function UserAccount() {
  ...
  const createToken = async (formValues) => {
    ...
    mapsApiClient
      .post<any>("/tokens/create", formValues)
      ...
      .then(function (createTokenResponse: any) {
        ...
        let newTokenModalContent = (
          <>
            ...
            <Group>
              <Text>Token:</Text>
              <Code color="blue" className={utils.codeFontSize}>
    ...
    <>
      ...
      >
        <Box className={utils.maxWidth}>
          ...
          />
          <Button type="submit" my="md" loading={creatingState}>
            Create API token
          </Button>
        </Box>
      </form>
