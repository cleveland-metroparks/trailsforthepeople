...
export function AuditLogView() {
  ...
  let logId = "";
  if (params.logId) {
    if (!isNaN(parseFloat(params.logId))) {
  ...
  const {
    ...
    queryKey: ["audit_log", params.logId],
    queryFn: () => getAuditLog(logId),
  });

  return (
    <div>
      <Anchor component={Link} to={`/logs`}>
        « Logs
      </Anchor>
              ...
              <Table.Td colSpan={4}>
                <Text fw={500} ta="center">
                  {logsIsError ? (
                    ...
                    >{`There was a problem fetching the logs data - ${logsError.message}`}</div>
                  ) : logsIsLoading ? (
