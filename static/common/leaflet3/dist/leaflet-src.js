...
L.LatLng.prototype = {
	...
	// GIN: bearingTo returns the compass bearing to get from this LatLng to a different LatLng
	// adapted from http://www.movable-type.co.uk/scripts/latlong.html
	bearingTo: function(/*LatLng*/ other)/*->Integer*/ {
	...
	// GIN: bearingWordTo wraps around bearingTo, to translate the bearing into letters such as N, NE, SW
	bearingWordTo: function(/*LatLng*/ other)/*->Integer*/ {
...
L.TileLayer = L.Class.extend({
	...
	options: {
		...
		//GIN, additions so we can identify layers by name and set their initial visibility state
		id: null,
		visibility: true
	},
	...
	// GIN: _setAutoZIndex and bringToFront and bringToBack were missing
...
L.TileLayer.WMS = L.TileLayer.extend({
	...
	// GIN: the addToBottom param was added to TileLayer but not here, silly
	// GIN: the 'srs' WMS param defaults to EPSG:3857 but most WMSs know Web Mercator as EPSG:900913 Allow the SRS to be overridden in options
	onAdd: function (map,addToBottom) {
		var projectionKey = (parseFloat(this.wmsParams.version) >= 1.3 ? 'crs' : 'srs');
		this.wmsParams[projectionKey] = this.options.crs ? this.options.crs.code : map.options.crs.code;
		L.TileLayer.prototype.onAdd.call(this, map, addToBottom);
	},

	// GIN: ability to change WMS params once the layer is loaded, vital for anything dynamic, filtered, or merged
	setWmsParam: function (name, value, redraw) {
		this.wmsParams[name] = value;
		if (redraw) this.redraw();
	},

	// GIN: a redraw() method, called by setWmsParam() and also callable separately
	redraw: function () {
		this._reset(true);
		this._update();
	},
	...
	getTileUrl: function (/*Point*/ tilePoint, /*Number*/ zoom)/*-> String*/ {
		...
		//GIN: pick a {s} substitution from the subdomains list
		var subdomains = this.options.subdomains;
...
L.Popup = L.Class.extend({
	...
	options: {
		minWidth: 50,
		maxWidth: 300,
		maxHeight: null, // GIN, support for maxHeight cribbed from leaflet 4
		autoPan: true,
		closeButton: true,
	...
	_updateLayout: function () {
		...
		this._contentNode.style.width = (width > this.options.maxWidth ? this.options.maxWidth : (width < this.options.minWidth ? this.options.minWidth : width)) + 'px';
		this._contentNode.style.whiteSpace = '';

		var height = this._container.offsetHeight;
		var maxHeight = this.options.maxHeight;
		var scrolledClass = 'leaflet-popup-scrolled';
		if (maxHeight && height > maxHeight) {
			this._contentNode.style.height = maxHeight + 'px';
			L.DomUtil.addClass(this._container, scrolledClass);
		} else {
			this._contentNode.style.height = '';
			L.DomUtil.removeClass(this._container, scrolledClass);
		}
...
L.Control.Zoom = L.Class.extend({
	onAdd: function (map) {
		...
		//GIN: add an event handler to this Control's map, so that a change of zoom can enable/disable the Zoom In and Zoom Out buttons
		this._map.on('zoomend', function () {
			var zoom = this._map.getZoom();

			if (zoom <= this._map.options.minZoom) {
				// zoomed out as far in as we can go; disable the - ZoomOut button
				L.DomUtil.addClass(this._zoomOutButton, 'disabled');
				L.DomUtil.addClass(this._zoomInButton, 'only');
			} else {
				L.DomUtil.removeClass(this._zoomOutButton, 'disabled');
				L.DomUtil.removeClass(this._zoomInButton, 'only');
			}

			if (zoom >= this._map.options.maxZoom) {
				// zoomed in as far in as we can go; disable the + ZoomIn button
				L.DomUtil.addClass(this._zoomInButton, 'disabled');
			} else {
				L.DomUtil.removeClass(this._zoomInButton, 'disabled');
			}
		}, this);
	},
	...
	_createButton: function (title, className, fn, context) {
		...
		/*GIN, we need the event propagation stopped even on Mobile: bad to zoom in and then trigger a click-query a second later, re-panning the map */
		//if (!L.Browser.touch) {
...
L.Map.include({
	locate: function (/*Object*/ options) {
		...
		if (options.watch) {
			this.locationWatch = navigator.geolocation.watchPosition(onResponse, onError, options);
		} else {
